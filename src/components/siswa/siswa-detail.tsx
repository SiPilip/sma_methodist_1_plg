...
export interface SiswaData {
  ...
  jurusan: string;
  kelas: string;
  tanggalLahir?: string; // Opsional
  motto?: string;       // Opsional
...
const formatTanggal = (tanggal: string | undefined) => {
  ...
  try {
    ...
    return "-"; // Return fallback jika tanggal tidak valid
  }
};
  ...
  return (
    <section className="dark:bg-[#495A87] bg-[#F1F2F4] min-h-screen py-10">
      <Container>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
            ...
            <div className="lg:col-span-4 bg-blue-50 dark:bg-black/20 p-8 flex flex-col items-center justify-center text-center border-b lg:border-b-0 lg:border-r border-gray-100 dark:border-white/10">
              <div className="relative w-48 h-48 mb-6 rounded-full overflow-hidden border-4 border-white shadow-lg ring-4 ring-blue-100 dark:ring-white/10">
                <Image 
                  src={data.foto || '/img/blank-profile-picture.webp'} 
                  alt={data.nama}
                  fill
            ...
            <div className="lg:col-span-8 p-8 flex flex-col justify-center">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
                ...
                <div className="flex items-start gap-3">
                  <div className="mt-1 p-2 bg-blue-50 dark:bg-white/5 rounded-lg text-blue-600 dark:text-blue-300"><HiIdentification size={24} /></div>
                  <div>
                ...
                <div className="flex items-start gap-3">
                  <div className="mt-1 p-2 bg-blue-50 dark:bg-white/5 rounded-lg text-blue-600 dark:text-blue-300"><HiUser size={24} /></div>
                  <div>
                ...
                <div className="flex items-start gap-3">
                  <div className="mt-1 p-2 bg-blue-50 dark:bg-white/5 rounded-lg text-blue-600 dark:text-blue-300"><HiCalendar size={24} /></div>
                  <div>
                    <p className={labelStyle}>Tanggal Lahir</p>
                    <p className={valueStyle}>{formatTanggal(data.tanggalLahir)}</p>
                  </div>
                </div>

                <div className="flex items-start gap-3">
                  <div className="mt-1 p-2 bg-blue-50 dark:bg-white/5 rounded-lg text-blue-600 dark:text-blue-300"><HiAcademicCap size={24} /></div>
                  <div>
        ...
        {/* --- BAGIAN 2: INFORMASI WALI KELAS (Render Kondisional) --- */}
        <div className="mt-10">
              ...
              <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 border-l-4 border-l-blue-600 rounded-r-xl shadow-sm hover:shadow-md transition-shadow duration-300 p-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Wali Kelas</p>
                      <h3 className="text-xl md:text-2xl font-bold text-blue-950 dark:text-white leading-tight">{data.waliKelas.nama}</h3>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
                    <div className="flex flex-col md:items-end">
                      <span className="text-[10px] md:text-xs font-semibold text-gray-400 uppercase mb-1">Kontak Darurat</span>
                      <a href={`tel:${data.waliKelas.telepon}`} className="flex items-center gap-2 text-base font-bold text-gray-700 dark:text-gray-200 hover:text-green-600 transition-colors">
                        <div className="p-1.5 bg-green-100 dark:bg-green-900/30 rounded-full text-green-600"><HiPhone size={14} /></div>
                        {data.waliKelas.telepon}
                      </a>
                    </div>
                    <Link href={data.waliKelas.linkProfile} className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-bold shadow-lg shadow-blue-200 dark:shadow-none transition-all active:scale-95 text-sm">
                      Lihat Profil
                    </Link>
                  </div>
                </div>
              </div>
              <p className="text-xs text-gray-400 mt-3 italic">* Hubungi wali kelas hanya pada jam kerja untuk keperluan akademik mendesak.</p>
            </>
          ) : (
            <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-6 flex items-center gap-4 text-gray-500 dark:text-gray-400 italic">
              <HiOutlineInformationCircle size={24} />
              <span>Informasi wali kelas tidak tersedia.</span>
            </div>
          )}
        </div>
      </Container>
